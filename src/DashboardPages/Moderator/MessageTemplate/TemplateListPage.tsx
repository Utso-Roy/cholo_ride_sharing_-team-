...
export const TemplateListPage: React.FC = () => {
  ...
  const [composeTmpl, setComposeTmpl] = useState<MessageTemplate | null>(null);
  ...
  useEffect(() => {
    fetch(); /* eslint-disable */
  }, [q, category, folder, pinnedOnly, page, pageSize]);
    ...
    <div className="flex items-center gap-2">
      ...
      <span className="p-input-icon-left">
        ...
        <InputText
          ...
          placeholder="Search title / body / key"
        />
      </span>
      <Dropdown
        value={category}
        onChange={(e) => setCategory(e.value)}
        options={["", "report", "dispute", "verification"]}
      ...
      <Dropdown
        ...
        onChange={(e) => setPinnedOnly(e.value)}
        options={[
  ...
  const actionsBody = (row: MessageTemplate) => (
    <div className="flex gap-2">
      <Button
        ...
        rounded
        text
        tooltip="Use / Quick Send"
        onClick={() => {
          setComposeTmpl(row);
          // variables ইনপুট init
          const initialVars: Record<string, string> = {};
          (row.variables || []).forEach((k) => (initialVars[k] = ""));
          setVars(initialVars);

          setSubject(row.currentVersion.subject || "");
          setBody(row.currentVersion.body || "");
          setTo("");
          setComposeOpen(true);
        }}
      />
      ...
      />
      <Button
        icon={row.pinned ? "pi pi-star-fill" : "pi pi-star"}
        rounded
        text
  ...
  const subjectBody = (row: MessageTemplate) => (
    <div>
      ...
      <div className="text-xs opacity-70">
        v{row.currentVersion.version} •{" "}
        {new Date(row.currentVersion.updatedAt).toLocaleString()}
      </div>
    </div>
  );
      ...
      <Dialog
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button label="Cancel" text onClick={() => setComposeOpen(false)} />
            <Button
              label="Send"
              onClick={async () => {
                if (!composeTmpl) return;
                if (!to.trim()) {
                  toast.current?.show({
                    severity: "warn",
                ...
                const missing = (composeTmpl.variables || []).filter(
                  (k) => !vars[k]
                );
                if (missing.length) {
                  toast.current?.show({
                ...
                try {
                  ...
                  await sendModerationMessage({
                    ...
                    body: rendered.body,
                    meta: {
                      templateId: composeTmpl._id,
                      version: composeTmpl.currentVersion.version,
                      variables: vars,
                    },
                  });
                  // optional usage mark
                  try {
                    await markTemplateUsed(composeTmpl._id);
                  } catch {}
                  toast.current?.show({
                    severity: "success",
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-3">
              <span className="p-float-label">
                ...
                />
                <label htmlFor="to">Recipient (email / phone)</label>
              </span>
              ...
              <span className="p-float-label">
                ...
                <label htmlFor="subject">
                  Subject (with &lbrace;&lbrace; vars &rbrace;&rbrace;)
                </label>
              </span>

              <div>
                <label className="block mb-1">
                  Body (with &lbrace;&lbrace; vars &rbrace;&rbrace;)
                </label>
                <InputTextarea
                  rows={8}
                  autoResize
              ...
              <div className="p-3 border rounded">
                ...
                {(composeTmpl.variables || []).map((k) => (
                  <div key={k} className="flex items-center gap-2 mb-2">
                    <label className="w-40 text-sm opacity-80">{`{{${k}}}`}</label>
                    <InputText
                      value={vars[k] ?? ""}
                      onChange={(e) =>
            ...
            <div className="p-3 border rounded">
              ...
              <div className="text-sm mb-2">
                <strong>Subject:</strong> {renderString(subject, vars)}
              </div>
              <pre className="text-sm whitespace-pre-wrap p-2 bg-surface-100 rounded">
                {renderString(body, vars)}
              </pre>
            </div>
          </div>
        )}
